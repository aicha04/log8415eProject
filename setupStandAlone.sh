...
sudo mysql -u root -e  "SOURCE /tmp/sakila-db/sakila-schema.sql; SOURCE /tmp/sakila-db/sakila-data.sql; USE sakila; create user user identified by 'password'; grant all on sakila.* to `user`@`%`;show grants for user;">>/var/log/user-data.log
